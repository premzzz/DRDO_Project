...
def normalize_braces(source: str) -> str:
    """
    Rewrites the source so every '{' and '}' that shares a line with other
    code gets pushed onto its own line. This guarantees our later line-based
    LOG_FLAG insertion always lands inside the block, never on a line that
    also contains the brace plus other statements.

    Example:
        for (int i = 0; i < 4; i++) {control_actuator(i);}
    becomes:
        for (int i = 0; i < 4; i++)
        {
        control_actuator(i);
    ...
    Uses the same tokeniser so braces inside strings/comments are never
    touched.
    """
    tokens = tokenise(source)
    edits = []   # (pos, "insert_before" | "insert_after")

    for t in tokens:
        if t["type"] not in ("LBRACE", "RBRACE"):
            continue

        pos = t["start"]

        # --- check character immediately before the brace on the same line ---
        line_start = source.rfind("\n", 0, pos) + 1
        before_text = source[line_start:pos]
        if before_text.strip() != "":
            # something non-whitespace precedes the brace on this line
            edits.append((pos, "before"))

        # --- check character immediately after the brace on the same line ---
        line_end = source.find("\n", t["end"])
        if line_end == -1:
            line_end = len(source)
        after_text = source[t["end"]:line_end]
        if after_text.strip() != "":
            edits.append((t["end"], "after"))
...
class Analyser:
    ...
    def __init__(self, source: str, file_stem: str):
        # Normalize first: push same-line braces onto their own lines so
        # later line-based LOG_FLAG insertion always lands correctly.
        source = normalize_braces(source)
    ...
    def parse_braced_block(self, context: str, keyword_pos: int = None) -> tuple[int, int]:
        """
        keyword_pos: character offset of the keyword (if/while/for/switch/else)
        that introduced this block. When provided, start_line in block_map is
        set to the keyword's line rather than the opening brace's line — so the
        expression line (e.g. 'if (x > 0) {') is correctly included in the
        block's range and gets the right color and count marker.
        """
        open_tok = self.cur()
        assert open_tok and open_tok["type"] == "LBRACE"
        open_pos = open_tok["start"]
        # Use keyword line as start if provided, otherwise fall back to brace line
        record_start_pos = keyword_pos if keyword_pos is not None else open_pos
        self.adv()   # consume {
            ...
            if t["type"] == "RBRACE":
                ...
                if context == "function":
                    if not has_return:
                        ...
                        self._record(flag, record_start_pos, close_pos)
                    else:
                        # extend the return-flag block to include the closing }
                        open_line = char_to_line(self.source, record_start_pos)
                        for entry in reversed(self.block_map):
                            if entry["flag"].startswith("func") and entry["start_line"] == open_line:
                ...
                kw = t["value"]
                kw_pos = t["start"]      # capture before adv()
                self.adv()

                if kw == "if":
                    self.parse_if_chain(kw_pos)
                elif kw == "while":
                    self.parse_while(kw_pos)
                elif kw == "for":
                    self.parse_for(kw_pos)
                elif kw == "do":
                    self.parse_do_while(kw_pos)
                elif kw == "switch":
                    self.parse_switch(kw_pos)
                elif kw == "return":
                    if context == "function":
                        flag = self._next("func")
                        ...
                        self.block_map.append({
                            "flag":       flag,
                            "start_line": char_to_line(self.source, record_start_pos),
    ...
    def parse_if_chain(self, keyword_pos: int = None):
        self.skip_trivia()
        ...
        if self.cur() and self.cur()["type"] == "LBRACE":
            self.parse_braced_block("if", keyword_pos)
        else:
            self.consume_to_semicolon()
    ...
    def _try_parse_else(self):
        ...
        if t and t["type"] == "KEYWORD" and t["value"] == "else":
            else_pos = t["start"]          # capture 'else' keyword position
            self.adv()
            self.skip_trivia()
            t2 = self.cur()
            if t2 and t2["type"] == "KEYWORD" and t2["value"] == "if":
                if_pos = t2["start"]       # capture 'if' keyword position
                self.adv()
                self.parse_if_chain(if_pos)
            elif t2 and t2["type"] == "LBRACE":
                self.parse_braced_block("else", else_pos)
            else:
                self.consume_to_semicolon()
        else:
            self.idx = saved
    ...
    def parse_while(self, keyword_pos: int = None):
        self.skip_trivia()
        if self.cur() and self.cur()["type"] == "LPAREN":
            self.read_parens()
        self.skip_trivia()
        if self.cur() and self.cur()["type"] == "LBRACE":
            self.parse_braced_block("while", keyword_pos)
        else:
            self.consume_to_semicolon()

    # ---- for ----------------------------------------------------------

    def parse_for(self, keyword_pos: int = None):
        self.skip_trivia()
        if self.cur() and self.cur()["type"] == "LPAREN":
            self.read_parens()
        self.skip_trivia()
        if self.cur() and self.cur()["type"] == "LBRACE":
            self.parse_braced_block("for", keyword_pos)
        else:
            self.consume_to_semicolon()

    # ---- do-while -----------------------------------------------------

    def parse_do_while(self, keyword_pos: int = None):
        self.skip_trivia()
        if self.cur() and self.cur()["type"] == "LBRACE":
            self.parse_braced_block("do", keyword_pos)
        self.skip_trivia()
    ...
    def parse_switch(self, keyword_pos: int = None):
        self.skip_trivia()
        if self.cur() and self.cur()["type"] == "LPAREN":
            self.read_parens()
        self.skip_trivia()
        if self.cur() and self.cur()["type"] == "LBRACE":
            self.parse_switch_body(keyword_pos)

    def parse_switch_body(self, keyword_pos: int = None):
        open_tok = self.cur()
        assert open_tok and open_tok["type"] == "LBRACE"
        open_pos = open_tok["start"]
        record_start_pos = keyword_pos if keyword_pos is not None else open_pos
    ...
    def run(self):
        ...
        while self.cur():
            ...
            if t["type"] == "KEYWORD":
                ...
                kw_pos = t["start"]      # capture before adv()
                self.adv()
                if kw == "if":        self.parse_if_chain(kw_pos)
                elif kw == "while":   self.parse_while(kw_pos)
                elif kw == "for":     self.parse_for(kw_pos)
                elif kw == "do":      self.parse_do_while(kw_pos)
                elif kw == "switch":  self.parse_switch(kw_pos)
                continue
            if t["value"] in TYPE_KEYWORDS or t["type"] == "OTHER":
                if self._looks_like_function_def():
                    # For functions, keyword_pos is the start of the return type —
                    # capture here before we consume tokens scanning for the brace
                    func_kw_pos = t["start"]
                    while self.cur() and self.cur()["type"] != "LBRACE":
                        self.adv()
                    if self.cur() and self.cur()["type"] == "LBRACE":
                        self.parse_braced_block("function", func_kw_pos)
                    continue
            self.adv()
